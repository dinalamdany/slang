...
param:
    var_type ID { VFormal(Datatype($1),Ident($2)) }
    | var_type ID LBRAC RBRAC {ArrFormal(Datatype($1),Ident($2))}
...
vdecl:
    ...
    | var_type ID LBRAC RBRAC { ArrDecl(Datatype($1),Ident($2))}
...
property:
    var_type ID {VarDecl(Datatype($1),Ident($2))}
    | var_type ID LBRAC RBRAC { ArrDecl(Datatype($1),Ident($2))} 
    | var_type ID COLON expr {VarAssignDecl(Datatype($1),Ident($2), $4)}
    | var_type ID LBRAC RBRAC ASSIGN LBRAC expr_list RBRAC {ArrAssignDecl(Datatype($1),Ident($2),$7)}
